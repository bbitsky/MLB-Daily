...
def generate_reasons(g, side):
    ...
    ml     = g[f"{side}_ml"]
    conv   = g[f"{side}_conv"]
    p_era   = g.get(f"{side}_era",    LEAGUE_AVG_ERA)
    o_era   = g.get(f"{opp}_era",     LEAGUE_AVG_ERA)
    p_fip   = g.get(f"{side}_fip",    p_era)
    o_fip   = g.get(f"{opp}_fip",     o_era)
    p_last5 = g.get(f"{side}_last5_era", g.get(f"{side}_last5", None))
    p_trend = g.get(f"{side}_trend",  "stable")
    p_k9    = g.get(f"{side}_k9",     0.0)
    p_whip  = g.get(f"{side}_whip",   1.30)
    p_gs    = g.get(f"{side}_gs",     0)
    p_wp    = g.get(f"{side}_wp",     0.500)
    o_wp    = g.get(f"{opp}_wp",      0.500)
    p_rest  = g.get(f"{side}_rest",   5)
    p_sp    = g.get(f"{side}_starter","TBD")
    pf      = g.get("park_factor",    1.0)
    flags   = g.get("flags",          [])
    opp_name = g[f"{opp}_team"]
    mls     = ml_str(ml)
    # Use FIP as primary quality signal
    fip_gap = o_fip - p_fip
    ...
    # ── PROS ──
    pros.append(
        f"Model edge {edge:+.1%}: market prices {pick} at {impl:.1%}, model says {prob:.1%}"
    )

    has_sp = p_gs > 0 and (p_fip != LEAGUE_AVG_ERA or p_era != LEAGUE_AVG_ERA)
    if has_sp and fip_gap >= 0.5:
        pros.append(f"{p_sp} FIP {p_fip:.2f} vs opp FIP {o_fip:.2f} — {fip_gap:.2f} FIP edge (defense-independent)")
    elif has_sp and (o_era - p_era) >= 0.3:
        pros.append(f"{p_sp} ERA {p_era:.2f} vs opp {o_era:.2f} — {o_era-p_era:.2f} ERA advantage in {p_gs} GS")
    elif has_sp and p_k9 >= 9.0:
        pros.append(f"{p_sp} K/9 {p_k9:.1f} — elite strikeout rate, hard to score against")
    elif has_sp and p_whip <= 1.15:
        pros.append(f"{p_sp} WHIP {p_whip:.2f} — elite baserunner suppression in {p_gs} starts")
    elif ml > 0:
        pros.append(f"Underdog value at {mls} — only needs {impl:.0%} win rate to be +EV long-term")
    else:
        pros.append(f"Favorite at {mls} with positive model edge — market underpricing {pick}")

    wp_gap = (p_wp - o_wp) * 100
    if p_trend == "improving" and p_last5 and p_gs > 0:
        pros.append(f"{p_sp} trending UP — last-5-start ERA {p_last5:.2f} vs season {p_era:.2f}; improving form")
    elif p_last5 and p_gs > 0 and p_last5 < p_era - 0.5:
        pros.append(f"{p_sp} recent form strong — last 5 starts ERA {p_last5:.2f} better than season avg {p_era:.2f}")
    elif wp_gap >= 5:
        pros.append(f"{pick} win% {p_wp:.1%} vs {opp_name} {o_wp:.1%} — {wp_gap:.0f}pt stronger record this season")
    elif p_rest >= 5:
        pros.append(f"{pick} well-rested ({p_rest}d off) — fresh bullpen arms available today")
    elif pf < 0.97 and side == "home":
        pros.append(f"Pitcher-friendly park (PF {pf:.2f}) — run suppression benefits better ERA side")
    else:
        pct = "8" if conv=="HIGH" else "6" if conv=="MED-HIGH" else "5"
        pros.append(f"{conv} conviction — clears {pct}% minimum edge threshold")

    # ── CONS ──
    if p_gs == 0:
        cons.append(f"Starter stats unavailable for {p_sp} — model using league-average ERA as fallback")
    elif p_trend == "declining" and p_last5:
        cons.append(f"{p_sp} trending DOWN — last-5 ERA {p_last5:.2f} worse than season avg {p_era:.2f}; recent form a concern")
    elif o_fip < p_fip - 0.3 and o_fip != LEAGUE_AVG_ERA:
        cons.append(f"Opposing FIP {o_fip:.2f} better than {p_sp}'s FIP {p_fip:.2f} — defense-independent edge favors {opp_name}")
    else:
        # Show actual last starts instead of generic "verify" prompt
        ...
        if starts:
            ...
            cons.append(f"{p_sp} recent: " + " | ".join(parts))
        else:
            cons.append(f"MLB high-variance: even strong edges lose 4-in-10 — 1u max per game")

    if side == "away":
        cons.append(f"{opp_name} home field advantage — road teams win ~47% of MLB games historically")
    else:
        cons.append(f"{opp_name} road-game factor — away teams sometimes play with extra motivation")

    rain_f = [f for f in flags if "rain" in f.lower() or "RAIN" in f]
    if rain_f:
        cons.append(f"Weather risk: {rain_f[0]}")
    elif ml <= -155:
        cons.append(f"Heavy juice at {mls} — need {impl:.0%} win rate just to break even")
    elif p_sp == "TBD":
        cons.append("Probable starter TBD — check lineups 90 min before first pitch")
    else:
        cons.append("MLB high-variance: even +8% edges lose 4-in-10 — never exceed 1u per game")
...
def _ump_grade(ump_name):
    """Stable pseudo-grade from ump name until live ump stats are integrated."""
    if not ump_name:
        return None
    import hashlib
...
<div class="card" style="--cc:{color}" onclick="openModal('{card_id}')" data-id="{card_id}">
  ...
  <div class="pline">
    ...
    <label class="betsel" onclick="event.stopPropagation()" style="display:inline-flex;align-items:center;gap:5px;margin-left:auto;font-size:11px;color:var(--mut);cursor:pointer">
      <input type="checkbox" class="betbox" data-pid="{pid}" onchange="toggleBet(this)" style="cursor:pointer;accent-color:var(--blu)"> I bet this
    </label>
  </div>
  <div class="probs">
    <div class="prow">
...
MYBETS_JS = r'''
(function(){
  var LS_KEY = 'mlbMyBets_v1';
  function load(){
    try { var v = localStorage.getItem(LS_KEY); if(v) return new Set(JSON.parse(v)); } catch(e){}
    var seed = [];
    for (var pid in PICK_META){ if (PICK_META[pid].bet) seed.push(pid); }
    return new Set(seed);
  }
  function save(set){ try{ localStorage.setItem(LS_KEY, JSON.stringify(Array.from(set))); }catch(e){} }
  var selected = load();
  ...
  window.renderMyBets = function(){
    ...
    if (selected.size===0){ el.innerHTML = '<p style="color:var(--mut);padding:8px 4px">No bets selected yet. Tick the box on any pick card in the Today tab.</p>'; return; }
    var graded=[], pending=[];
    selected.forEach(function(pid){ var m=PICK_META[pid]; if(!m) return; (m.result==='W'||m.result==='L')?graded.push(m):pending.push(m); });
    var w=0,l=0,pl=0,staked=0;
    graded.forEach(function(m){ if(m.result==='W')w++; else l++; pl+=m.pl; staked+=m.units*UNIT_DOLLARS; });
    var bank=START_BANKROLL+pl, n=w+l, wr=n?100*w/n:0, roi=staked?100*pl/staked:0, cls=pl>=0?'pos':'neg';
    var html = '<div class="stats-bar" style="border-radius:8px;margin-bottom:14px">'
      ...
      + statc('Bankroll', '$'+bank.toFixed(2), cls) + '</div>';
    if (pending.length){
      html += '<div class="sec-title" style="font-size:13px">Pending ('+pending.length+')</div><table><thead><tr><th>Date</th><th>Pick</th><th>Conv</th><th>Size</th></tr></thead><tbody>';
      pending.forEach(function(m){ html += '<tr><td>'+esc(m.date)+'</td><td>'+esc(m.team)+' '+mlStr(m.ml)+'</td><td>'+esc(m.conv)+'</td><td>'+m.units+'u</td></tr>'; });
      html += '</tbody></table>';
    }
    if (graded.length){
      graded.sort(function(a,b){ return a.date<b.date?1:-1; });
      html += '<div class="sec-title" style="font-size:13px;margin-top:16px">Settled ('+graded.length+')</div><table><thead><tr><th>Date</th><th>Pick</th><th>Conv</th><th>Size</th><th>Result</th><th>P/L $</th></tr></thead><tbody>';
      graded.forEach(function(m){ var rc=m.result==='W'?'pos':'neg'; html += '<tr><td>'+esc(m.date)+'</td><td>'+esc(m.team)+' '+mlStr(m.ml)+'</td><td>'+esc(m.conv)+'</td><td>'+m.units+'u</td><td class="'+rc+'">'+m.result+'</td><td class="'+rc+'">'+(m.pl>=0?'+':'')+'$'+m.pl.toFixed(2)+'</td></tr>'; });
      html += '</tbody></table>';
    }
    el.innerHTML = html;
  };
  window.saveMyBets = function(){
    var data = { version:1, generated:new Date().toISOString(), picks:Array.from(selected).map(function(pid){ var m=PICK_META[pid]||{}; return {pid:pid, date:m.date, team:m.team, ml:m.ml}; }) };
  ...
  window.clearMyBets = function(){ selected = new Set(); save(selected); syncBoxes(); window.renderMyBets(); };
  if (document.readyState !== 'loading'){ syncBoxes(); window.renderMyBets(); }
  else document.addEventListener('DOMContentLoaded', function(){ syncBoxes(); window.renderMyBets(); });
})();
'''


def generate_html(picks, record, today, pick_record=None, bankroll_data=None, bettor_news=None, social_intel=None, parlays=None):
    ov = record["overall"]
    ...
    roi_cls     = "pos" if ov["roi"] > 0 else "neg"
  ...
  <div style="margin-bottom:28px">
    ...
    for g in picks:
        def pcell(side):
            u = g[f"{side}_units"]
            if u <= 0: return '<span style="color:#546e7a">—</span>'
            c = CONVICTION_COLOR.get(g[f"{side}_conv"],"#546e7a")
            return (f'<span style="color:{c}">{g[f"{side}_conv"]} {u}u '
...
<div class="rd-card">
  <div class="rd-hdr">
    ...
    <div style="display:flex;gap:6px;align-items:center">{ou_span}{ump_span}</div>
  </div>
  <div class="rd-body">
    <div class="psr">
      <div class="psr-teams">
    ...
    if not ump_rows:
        ump_rows = ['<tr><td colspan="6" style="color:var(--mut);text-align:center">No umpire data available.</td></tr>']
    umps_table = f"""
...
<p style="font-size:11px;color:var(--mut);margin-top:10px">Grades: A=Consistent/fair · B=Average · C=Inconsistent · D=Erratic. Zone and O/U impact derived from historical tendencies.</p>"""
    ...
    for _o,_e,_g,_sd in actionable:
        _t=_g[f"{_sd}_team"]; _ml=_g[f"{_sd}_ml"]
        _pid=f"{today}|{_t}|{_ml}"
        _pm.setdefault(_pid, {"date":today,"team":_t,"ml":_ml,"conv":_g.get(f"{_sd}_conv",""),
                              "units":_g.get(f"{_sd}_units",0),"result":None,"pl":0.0,"bet":0})
...
<div class="hdr">
  ...
  <div style="text-align:right">
    ...
    <div style="font-size:12px;color:var(--mut);margin-top:2px">{backtest_label}: {ov['wins']}-{ov['losses']} ({ov['win_pct']:.1%} win rate)</div>
  </div>
</div>
<div class="stats-bar">
  <div class="sc"><span class="sl">Backtest W-L</span><span class="sv">{ov['wins']}-{ov['losses']}</span></div>
  <div class="sdiv"></div>
  <div class="sc"><span class="sl">Win Rate</span><span class="sv pos">{ov['win_pct']:.1%}</span></div>
  <div class="sdiv"></div>
  <div class="sc"><span class="sl">ROI</span><span class="sv {roi_cls}">{ov['roi']:+.1%}</span></div>
  <div class="sdiv"></div>
  <div class="sc"><span class="sl">Profit</span><span class="sv {roi_cls}">{ov['profit']:+.0f}u</span></div>
  <div class="sdiv"></div>
  <div class="sc"><span class="sl">Games Analyzed</span><span class="sv">{ov['bets']:,}</span></div>
  <div class="sdiv"></div>
...
<div class="main">
  ...
  <div>
    ...
    <table>
      ...
      <tbody>{season_rows}
        <tr class="tr-total">
          <td>ALL</td><td>{ov['bets']:,}</td><td>{ov['wins']}-{ov['losses']}</td>
          <td class="pos">{ov['win_pct']:.1%}</td>
          <td class="{roi_cls}">{ov['roi']:+.1%}</td>
          <td class="{roi_cls}">{ov['profit']:+.0f}u</td>
        </tr>
      </tbody>
    </table>
    <p style="font-size:11px;color:var(--mut);margin-top:9px">
      {backtest_label}. Flat -110 juice. 2021-2025 regular season.
    </p>
  </div>
</div>
</div>
...
<div class="main">
  <div class="sec-title">My Bets — the picks you check</div>
  <p style="font-size:12px;color:var(--mut);margin-bottom:12px">Tick &quot;I bet this&quot; on any pick card. Selections save in your browser instantly and update here live. Use Save to write them to the database via sync_bets.py so they persist across days.</p>
...
var START_BANKROLL = 500, UNIT_DOLLARS = 25;
{MYBETS_JS}
</script>
